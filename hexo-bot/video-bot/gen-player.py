...
# -*- coding: utf-8 -*-
"""
生成 player.html（独立脚本，数据从 data/all.js 加载）
支持 Hexo Butterfly 主题亮色/暗色切换
"""
import os
import json
...
if not os.path.exists(all_js):
    combined = {}
    for f in os.listdir(DATA_DIR):
        if f.endswith('.json') and f != 'index.json':
            try:
                with open(os.path.join(DATA_DIR, f), 'r', encoding='utf-8') as fh:
                    combined[f] = json.load(fh)
            except:
                pass
    with open(all_js, 'w', encoding='utf-8') as fh:
        fh.write(f'var TVBOX_DATA = {json.dumps(combined, ensure_ascii=False)};')

html = r'''<!DOCTYPE html><html lang="zh-CN" data-theme="dark">
<head>
<meta charset="UTF-8">
...
.next-btn:disabled{opacity:0.4;cursor:default;background:var(--bg-toolbar)}
...
<div class="player-box">
  ...
  <button id="nextBtn" class="next-btn" onclick="playNext()" style="display:none">下一集 &gt;</button>
</div>
...
function loadIndex() {
  ...
  names.sort().forEach(function(k) {
    var d = TVBOX_DATA[k];
    ta += d.total_episodes||0;
    pa += d.playable_count||0;
...
function render() {
  ...
  pg += '<button onclick="go(' + pages + ')"' + (currentPage>=pages?' disabled':'') + '>\u00bb</button>';
...
function playEp(el) {
  ...
  document.getElementById('nextBtn').style.display = currentEpNext ? 'block' : 'none';
  doPlay(url, name);
}
...
HEXO_VIDEO_DIR = os.path.normpath(HEXO_VIDEO_DIR)  # D:\hexo\source\video
